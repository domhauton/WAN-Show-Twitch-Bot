  ...
  private BotCommandResult messageRepeatWithConvert(ImmutableList<String> args,
    ...
    if (optionalURL.isPresent()) {
      try {
        ...
        log.info("Sending converted com.domhauton.wanbot.url {} to channel {} times", convertedURL, linkRepeatCount);
        return new BotCommandResult(
            getTwitchMessages(channelManager, convertedURL, linkRepeatCount),
            "Your com.domhauton.wanbot.url was converted and repeated " + linkRepeatCount + " times. " + convertedURL);
      } catch (ChannelOperationException e) {
        log.info("Failed to convert com.domhauton.wanbot.url: {}", optionalURL::get);
        throw new BotCommandException("Try with flag 'f' + Failed to convert URL " + optionalURL);
      }
    } else {
      log.info("Could not repeat given string as not com.domhauton.wanbot.url: {}", optionalURL::get);
